...
export default function SideBar() {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <div className={`flex flex-col items-center mt-8 gap-2 transition-all duration-500 ${isCollapsed ? 'space-y-4' : 'items-stretch'}`}>
        {/* Example nav items */}
        <button className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} p-2 rounded-lg hover:bg-[--tertiary] transition-all duration-200`}>
          <div className="w-5 h-5 bg-[--text-secondary] rounded" />
        ...
        <button className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} p-2 rounded-lg hover:bg-[--tertiary] transition-all duration-200`}>
          <div className="w-5 h-5 bg-[--text-secondary] rounded" />
          <span className={`text-[--text-secondary] transition-opacity duration-300 ${isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100 w-auto ml-2'}`}>Settings</span>
        </button>
      </div>
